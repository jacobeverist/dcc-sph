...
// `test_prediction_improves_on_repeating_sequence` used to live here. It asserted
// that accuracy does not regress with more training — a learning outcome, which is
// not a CI gate in this repository. It moved to `tests/learning.rs`, which
// `cargo test` does not run; see that file's header for the reasoning.
